...
This guide covers setting up Daimo Pay for crypto-to-mana purchases on Oracle using Daimo's session-based SDK.
...
curl -X POST https://api.daimo.com/v1/webhooks \
  ...
  -d '{
    "url": "https://api.oracle.markets/daimo-webhook",
    "events": ["session.succeeded"],
    "description": "Oracle mana fulfillment"
  }'
```
...
| ----------- | ------------------------------------------------ |
| Dev         | `https://api.dev.oracle.markets/daimo-webhook` |
| Prod        | `https://api.oracle.markets/daimo-webhook`     |

## 3. Set Up Secrets (GCP Secret Manager)

Add the following secrets to GCP Secret Manager:

- **Dev**: [dev-oracle secrets](https://console.cloud.google.com/security/secret-manager?project=dev-oracle)
- **Prod**: [oracle secrets](https://console.cloud.google.com/security/secret-manager?project=oracle)
...
- **Dev logs**: [GCP Logs Explorer (dev)](https://console.cloud.google.com/logs/query?project=dev-oracle)
- **Prod logs**: [GCP Logs Explorer (prod)](https://console.cloud.google.com/logs/query?project=oracle)
