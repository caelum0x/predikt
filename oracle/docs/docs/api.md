...
Programmatic access to [Oracle](https://oracle.markets).

:::caution

Our API was historically hosted on `https://oracle.markets/api`, but we recently moved to give the API its own domain at `api.oracle.markets`.
...
If you notice any errors or omissions in this documentation, please let us know on Discord, or fix it yourself by [submitting a pull request](https://github.com/oracle/manifold/blob/main/docs/docs/api.md).
...
```bash
curl "https://api.oracle.markets/v0/markets?limit=1" -X GET
```

You can also [go to the url directly](https://api.oracle.markets/v0/markets?limit=1) in your browser to see the response of any GET request.
...
```bash
 curl "https://api.oracle.markets/v0/bet" -X POST -H 'Content-Type: application/json' \
    -H 'Authorization: Key {...}' \
    --data-raw '{"amount":1, \
                 "outcome":"YES", \
...
- `Authorization: Key {key}`. A Oracle API key associated with a user
...
Use of the Oracle API is subject to our [Terms of Service](./terms). Key points:

- **Permitted uses**: Building bots, automated trading systems, algorithmic tools, and integrations that interact with Oracle.
- **Prohibited uses**: Scraping the site through means other than the API; circumventing rate limits or security measures.
- **AI training data**: You may not use API data to train AI/ML models for commercial purposes without obtaining a data license from us. Academic research, personal projects, and non-commercial use are permitted. Contact **data@oracle.markets** for commercial licensing.
...
[common/src/api/schema.ts](https://github.com/oracle/manifold/tree/main/common/src/api/schema.ts) - type definitions of the apis, including parameters and return values.
...
type User = {
  ...
  isBot?: boolean
  isAdmin?: boolean // is Oracle team
...
[
  {
    ...
    "url":"https://oracle.markets/Austin/will-i-write-a-new-blog-post-today",
...
type LiteMarket = {
  ...
  // Note: This url always points to https://oracle.markets, regardless of what instance the api is running on.
  // This url includes the creator's username, but this doesn't need to be correct when constructing valid URLs.
  //   i.e. https://oracle.markets/Austin/test-market is the same as https://oracle.markets/foo/test-market
  url: string
...
```bash
curl "https://api.oracle.markets/v0/market/3zspH9sSzMlbFQLn9GKR" -X GET
```
...
```json
{
  ...
  "creatorAvatarUrl": "https://firebasestorage.googleapis.com/v0/b/oracle.appspot.com/o/user-images%2FAngela%2F50463444807_edfd4598d6_o.jpeg?alt=media&token=ef44e13b-2e6c-4498-b9c4-8e38bdaf1476",
  ...
  "url": "https://oracle.markets/Angela/what-is-good",
  "pool": null,
  ...
  "answers": [
    {
      ...
      "avatarUrl": "https://firebasestorage.googleapis.com/v0/b/oracle.appspot.com/o/user-images%2FAngela%2F50463444807_edfd4598d6_o.jpeg?alt=media&token=ef44e13b-2e6c-4498-b9c4-8e38bdaf1476",
      "id": "1",
      "username": "Angela",
      "number": 1,
      "name": "Angela",
    ...
    },
    {
      ...
      "id": "3",
      "avatarUrl": "https://firebasestorage.googleapis.com/v0/b/oracle.appspot.com/o/user-images%2FMartin%2Fgiphy.gif?alt=media&token=422ef610-553f-47e3-bf6f-c0c5cc16c70a",
      "text": "Toyota Camry",
      "contractId": "lEoqtnDgJzft6apSKzYK",
      "name": "Undox",
      "username": "Undox",
...
```bash
curl "https://api.oracle.markets/v0/slug/will-carrick-flynn-win-the-general" -X GET
```
...
Search or filter markets, Similar to the [browse page](https://oracle.markets/browse).
...
```bash
curl https://api.oracle.markets/v0/search-markets?term=biden&sort=liquidity&filter=resolved&contractType=BINARY&limit=2 -X GET
```
...
```bash
# Page 1
curl "https://api.oracle.markets/v0/search-markets?sort=newest&limit=100"
# Page 2: pass the createdTime of the last result from page 1
curl "https://api.oracle.markets/v0/search-markets?sort=newest&limit=100&beforeTime=1706000000000"
```
...
```bash
curl "https://api.oracle.markets/v0/market/9t61v9e7x4/prob" -X GET
```
...
```bash
curl "https://api.oracle.markets/v0/market-probs?ids=9t61v9e7x4&ids=ZNlNdzz690" -X GET
```
...
```bash
curl "https://api.oracle.markets/v0/market/kupKInoLsjMuiDiNfogm/positions?top=1&bottom=1" -X GET
```
        ...
        "xv86CDBe0flxF2epvO3f": [
            {
                ...
                "userAvatarUrl": "https://firebasestorage.googleapis.com/v0/b/oracle.appspot.com/o/user-images%2Fian%2Fm6zfu88qLr.png?alt=media&token=126eab7e-fcba-49bd-b2e2-669b3fa9eaf0",
    ...
    "contracts": [
        {
            ...
            "coverImageUrl": "https://storage.googleapis.com/oracle.appspot.com/contract-images/PedroSobral/112df20121f7.jpg",
            "volume24Hours": 0,
            "freshnessScore": 0,
...
```bash
curl "https://api.oracle.markets/v0/users?limit=1" -X GET
```
...
[
  {
    ...
    "name": "Austin",
    "username": "Austin",
    "url": "https://oracle.markets/Austin",
    "avatarUrl": "https://lh3.googleusercontent.com/a-/AOh14GiZyl1lBehuBMGyJYJhZd-N-mstaUtgE4xdI22lLw=s96-c",
    "bio": "I build Oracle! Always happy to chat; reach out on Discord or find a time on https://calendly.com/austinchen/manifold!",
...
```bash
curl -X POST https://api.oracle.markets/v0/bet \
...
```bash
curl -X POST https://api.oracle.markets/v0/multi-bet \
  -H 'Authorization: Key YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
...
```bash
curl "https://api.oracle.markets/v0/bets?username=Oracle&contractSlug=will-i-be-able-to-place-a-limit-ord" -X GET
```
...
```bash
curl "https://api.oracle.markets/v0/txns?limit=10&category=MANA_PAYMENT" -X GET
```
...
```bash
curl "https://api.oracle.markets/v0/get-boost-history?limit=50&includePending=true"
```

## Websockets

Oracle provides a real-time websocket server that allows you to subscribe to updates about markets, bets, and other events. The websocket endpoint is available at `wss://api.oracle.markets/ws` and `wss://api.dev.oracle.markets/ws`.
...
const client = new APIRealtimeClient('wss://api.oracle.markets/ws')
...
```javascript
const ws = new WebSocket('wss://api.oracle.markets/ws')
let txid = 0
...
Oracle has some internal API endpoints that are not part of the official API. These are largely undocumented, but a few are mentioned here for third-party use until a more permanent solution is implimented. These endpoints are not preceeded by `/v0` and are even more subject to sudden changes than the official API endpoints.
...
- 2023-12-18: `oracle.markets/api` -> `api.oracle.markets`. Please migrate old code.
