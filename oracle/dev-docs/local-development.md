...
How to run Oracle locally without Firebase or GCP credentials by using `LOCAL_ONLY` mode.

This mode is not explicitly supported by Oracle, and may break as we change things in the future.
...
pass should show only 2 skipped (Oracle Love types not needed for local dev).
