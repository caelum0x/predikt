...
import { DEV_WEB_URL, WEB_URL } from 'lib/config'
...
// Sentry DSN is read from EXPO_PUBLIC_SENTRY_DSN so no Manifold/foreign DSN is
// committed. When unset, Sentry is effectively a no-op (no events are sent).
// TODO(SHIP): set EXPO_PUBLIC_SENTRY_DSN to the real Predikt project DSN, or
// remove Sentry entirely (see app.config.js + sentry.properties).
Sentry.init({
...
// Points the WebView shell at the Predikt web deployment. Both origins are
// derived from the single DOMAIN constant in lib/config.ts; the prod origin can
// be overridden at build/runtime via EXPO_PUBLIC_WEB_URL.
const BASE_URI = ENV === 'DEV' ? DEV_WEB_URL : WEB_URL

// NOTE: the notification handler is configured once in init.ts (imported below
// via ./init) so behavior is deterministic. Do not add a second
// Notifications.setNotificationHandler here — a duplicate with conflicting
// settings would non-deterministically override the init.ts one.
...
const App = () => {
  ...
  // Subscribe inside an effect (not the render body) to avoid re-subscribing on
  // every render and leaking listeners; return the unsubscribe as cleanup.
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((user) =>
      user ? setFbUser(user) : null
    )
    return unsubscribe
  }, [])
  ...
  const getPushToken = async () => {
    ...
    // Without a real EAS project id, getExpoPushTokenAsync cannot mint a token.
    // Guard against the unconfigured placeholder so this path fails softly
    // (returns null) instead of throwing in callers without a catch, e.g.
    // 'tryToGetPushTokenWithoutPrompt'. See app.config.js EAS_PROJECT_ID.
    if (!projectId || projectId === 'REPLACE_WITH_EAS_PROJECT_ID') {
      log('Missing EAS projectId; cannot get Expo push token')
      return null
    }
