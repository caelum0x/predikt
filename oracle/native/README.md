...
- You need to make sure the Predikt team is set as the signer for app builds in Xcode - open up the ios directory in Xcode and set the signing account.
- `ios/` and `android/` are generated: run `npx expo prebuild --clean` before building (see FIREBASE-SETUP.md). They are gitignored.
...
- Want to see app logs of a production build? (Only works on android):
  - `$ adb logcat --pid=$(adb shell pidof -s com.predikt.app)`

## Building

- You'll need Predikt's own Android signing credentials to sign android builds for the Google Play store, and to be added to the Predikt Apple Developer team to build iOS apps.
...
- The dev and prod version of the app use the same application id (`com.predikt.app`). This may not be not ideal but it works.
- Notifications on android dev won't work unless the Firebase server signing key is linked to the application id (`com.predikt.app`). To fully separate you'd need a distinct application id for dev.
- Try out sending push notifications [here](https://expo.dev/notifications) (using the `pushToken` from your `private-user` object)
- Manage releases in the Predikt Google Play Console.

## Monitoring

- Sentry: set to the Predikt Sentry org (see `REPLACE_WITH_SENTRY_ORG` in `app.config.js`), or remove Sentry if unused. See FIREBASE-SETUP.md.
...
- `error: cannot find symbol import ...BuildConfig;`
  - Ensure `namespace "com.predikt.app"` is set in `app/build.gradle`
  - Ensure the package declaration at the top of any generated Java/Kotlin source matches `com.predikt.app`

## Troubleshooting

- getting an errors on build/install? like `Error: spawn .../native/android/gradlew EACCES`
