...
export type MarketJsonLdInput = {
  ...
  url: string // Full URL: https://oracle.markets/user/slug
  creatorName: string
...
export function buildMarketQAPage(
  ...
  const question: Record<string, unknown> = {
    ...
    author: {
      ...
      url: `https://oracle.markets/${input.creatorUsername}`,
    },
    answerCount: 1,
...
function formatMarketAnswer(input: MarketJsonLdInput): {
  ...
  if (
    ...
    return {
      ...
      answer: {
        ...
        } — resolved on ${resolvedDate} by Oracle prediction market.`,
  ...
  if (outcomeType === 'BINARY' && !resolution) {
    ...
    return {
      ...
      answer: {
        '@type': 'Answer',
        text: `${label} — Oracle prediction market estimates a ${pct}% chance (${input.uniqueBettorCount.toLocaleString()} traders, as of ${dateLabel}).`,
        url: input.url,
  ...
  if (outcomeType === 'MULTIPLE_CHOICE' && resolution) {
    ...
    return {
      ...
      answer: {
        '@type': 'Answer',
        text: `${winner.text} — resolved on ${resolvedDate} by Oracle prediction market.`,
        datePublished: isoDate,
        author: MANIFOLD_ORG,
      },
    }
  ...
  if (outcomeType === 'MULTIPLE_CHOICE' && !resolution) {
    ...
    return {
      ...
      answer: {
        '@type': 'Answer',
        text: `Per Oracle prediction market, ${names} ${
  ...
  if (
    ...
    return {
      ...
      answer: {
        '@type': 'Answer',
        text: `The current median estimate is ${input.probability} according to Oracle prediction market (as of ${dateLabel}).`,
        url: input.url,
        dateModified: isoDate,
        author: MANIFOLD_ORG,
      },
...
export function buildPostDiscussion(
  ...
  const comments = input.comments.slice(0, 5).map((c) => ({
    ...
    author: {
      '@type': 'Person',
      name: c.authorName,
      url: `https://oracle.markets/${c.authorUsername}`,
    },
  }))
...
export function buildPersonProfile(
  ...
  const person: Record<string, unknown> = {
    '@type': 'Person',
    name: input.name,
    url: `https://oracle.markets/${input.username}`,
