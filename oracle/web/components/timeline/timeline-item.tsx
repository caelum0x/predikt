...
export const TimelineItem = ({
  ...
  const itemContent = (
    <div className="flex items-center rounded-full px-2 py-1 transition-all hover:bg-fuchsia-100/60 hover:shadow-md dark:hover:bg-fuchsia-900/30 sm:px-2.5">
      ...
      <span
        className="max-w-[150px] truncate whitespace-nowrap text-sm font-medium text-ink-900 sm:max-w-[200px] sm:text-base"
        title={item.title}
      >
        {item.title}
      </span>
      {item.probability !== undefined && (
        <span className="ml-1 text-xs text-ink-600 sm:ml-1.5 sm:text-sm">
