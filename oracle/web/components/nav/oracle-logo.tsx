...
export function OracleLogo(props: { className?: string; twoLine?: boolean }) {
    ...
    <div className="flex items-center gap-2">
      ...
      >
        {aprilFools ? (
          ...
          <LogoIcon
            className="h-10 w-10 shrink-0 stroke-primary-700 transition-transform group-hover:rotate-12 dark:stroke-white"
            aria-hidden
          />
        )}
        <div
          className={clsx('text-xl font-thin text-primary-700 dark:text-white')}
        >
          {aprilFools ? 'MANIFOOLD' : ENV == 'DEV' ? 'DEVIFOLD' : 'PREDIKT'}
        </div>
      </Link>
    </div>
  )
