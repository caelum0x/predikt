...
import { useCreatorEarnings } from 'web/hooks/use-creator-earnings'
...
export function UserContractsList(props: {
  ...
  const user = useUser()
  const isOwnProfile = creator.id === user?.id

  // Real trader-bonus earnings from this creator's markets. This is a personal
  // ledger (bank -> creator txns), so only surface it on the creator's own
  // profile. Undefined while loading; we never render a fabricated fallback.
  const earnings = useCreatorEarnings(isOwnProfile ? creator.id : undefined)
  ...
  return (
    <Col className={'w-full'}>
      <Row className={'mb-4 gap-8'}>
        ...
        />
        {isOwnProfile && earnings !== undefined && (
          <MarketStats
            title={
              <Row className="items-center gap-1">
                <CashIcon className="h-4 w-4" aria-hidden />
                Earned
              </Row>
            }
            total={formatMoney(earnings.totalMana)}
            subTitle={
              <Tooltip text={'Trader bonuses paid to you across your markets'}>
                <InfoTooltip
                  text={
                    'You earn a bonus every time a new trader bets on a market you created.'
                  }
                  size="sm"
                />
              </Tooltip>
