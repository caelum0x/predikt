...
import {
  matchOracleMarketUrl,
  replaceOracleUrlWithMention,
  replaceUrlWithTitleLink,
} from 'web/components/editor/insert-oracle-mention'
  ...
  return (
    ...
    >
      ...
      <a
        ...
        key={id}
        href={url}
        target={url.includes('oracle.markets') ? '_self' : '_blank'}
        rel="noreferrer ugc"
      >
        <img
...
export const insertLinkPreviews = async (
  ...
  await Promise.all(
    links
      ...
      .map(async (link) => {
        try {
          // Oracle market URLs become inline contract-mention widgets
          // (same as typing % and picking from the dropdown).
          const manifold = matchOracleMarketUrl(link)
          if (manifold) {
            await replaceOracleUrlWithMention(editor, link, manifold.slug)
