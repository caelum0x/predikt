...
const isOracleHost = (hostname: string) =>
  hostname === 'oracle.markets' ||
  hostname === 'manifold.love' ||
  hostname === 'localhost' ||
  hostname.endsWith('.oracle.markets') ||
  hostname.endsWith('.manifold.love')

const SLUG_REGEX = /^[a-z0-9-]+$/i

export const matchOracleMarketUrl = (url: string) => {
  try {
    const u = new URL(url.trim())
    if (!isOracleHost(u.hostname)) return null
...
// Replace a pasted Oracle market URL with an inline contract-mention node
// (the same widget produced by typing `%` and picking from the dropdown).
export const replaceOracleUrlWithMention = async (
  editor: Editor,
  url: string,
  slug: string
) => {
