...
import { AboutOracle } from './about-oracle'
...
export const ExplainerPanel = (props: {
  className?: string
  showWhatIsOracle?: boolean
  showAccuracy?: boolean
}) => {
  const { className, showWhatIsOracle = true, showAccuracy = true } = props
  const handleSectionClick = (sectionTitle: string) => {
    track('explainer section click', { sectionTitle })
  }
  return (
    <Col className={clsx('max-w-xl', className)}>
      {showWhatIsOracle && <WhatIsOracle onClick={handleSectionClick} />}
...
const WhatIsOracle = ({
  ...
  <ExpandSection
    title={
      <>
        <GoGraph className="mr-2  " /> What is Predikt?
      </>
    }
    onClick={() => onClick('What is Predikt?')}
  >
    <AboutOracle />
  </ExpandSection>
)
  ...
  >
    <div className="pb-2">
      ...
      <a
        ...
        target="_blank"
        href="https://oracle.markets/calibration"
      >
    ...
    <div className="pb-2">
      ...
      like {TRADING_TERM} still use Predikt to get reliable news.
    </div>
    <div></div>
  </ExpandSection>
)
...
const CashPrizes = ({
  ...
  >
    <div className="pb-2">
      Predikt runs regular prize drawings where you can win real USDC.
    </div>
    <div className="pb-2">
