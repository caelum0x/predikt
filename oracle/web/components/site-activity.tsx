...
export function SiteActivity(props: {
  className?: string
  // When set, the feed opens pinned to this filter (e.g. a dedicated
  // "following" surface that shows only the people you follow).
  defaultFilterMode?: FilterMode
}) {
  const { className, defaultFilterMode } = props
    ...
    usePersistentLocalState<ActivityState>(
      {
        ...
        filterMode: defaultFilterMode ?? 'custom-topics',
      },
      defaultFilterMode
        ? `site-activity-state-3-${defaultFilterMode}`
        : 'site-activity-state-3'
    )

  // A pinned surface (e.g. /following) always forces its filter mode, even if
  // the user previously changed it in the general activity feed.
  useEffect(() => {
    if (defaultFilterMode && activityState.filterMode !== defaultFilterMode) {
      setActivityState((prev) => ({ ...prev, filterMode: defaultFilterMode }))
    }
