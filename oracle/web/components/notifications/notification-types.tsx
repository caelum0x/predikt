...
function SignupBonusNotification(props: {
  ...
  const text = (
    <span>
      Thanks for using Predikt! We sent you{' '}
...
export function MarketResolvedNotification(props: {
  ...
  const showOracleAsResolver = token === 'CASH'

  const resolverName = showOracleAsResolver
    ? MANIFOLD_USER_NAME
    : resolvedByAdmin
    ? 'A mod'
    : sourceUserName
  const resolverUsername = showOracleAsResolver
    ? MANIFOLD_USER_USERNAME
    : sourceUserUsername
  const resolverAvatarUrl = showOracleAsResolver
    ? MANIFOLD_AVATAR_URL
    ...
    >
      <span>
        ...
        <span
          className={clsx(
            rating > 3
              ? 'rounded-md bg-gradient-to-br from-amber-100 to-amber-400 px-2 dark:text-ink-500'
              : ''
          )}
        >
...
function AirdropNotification(props: {
  ...
  return (
    ...
    >
      ...
      Predikt for being active for 30 days this year!
    </NotificationFrame>
  )
}
  ...
  return (
    <NotificationFrame
      ...
      icon={
        <GiftIcon
          className={
            isShipped ? 'text-primary-500 h-8 w-8' : 'text-ink-500 h-8 w-8'
          }
        />
      }
      link="/shop"
