...
import { createOnchainMarket } from 'web/lib/onchain/market'
import { useOnchainWallet } from 'web/hooks/use-onchain-wallet'
import { ConnectWalletSheet } from '../onchain/connect-wallet-sheet'
import { CubeIcon, CollectionIcon } from '@heroicons/react/solid'
import Link from 'next/link'
import { AiMarketComposer } from '../create/ai-market-composer'
...
import { ResolutionCriteriaSection } from './resolution-criteria-section'
...
export function NewContractPanel(props: {
  ...
  const getDefaultFormState = (): FormState => ({
    ...
    min: params?.min,
    max: params?.max,
    // DATE markets carry their range as date strings (dateMin/dateMax); other
    // ranged types carry numeric min/max.
    minString: params?.dateMin || params?.min?.toString() || '',
    maxString: params?.dateMax || params?.max?.toString() || '',
    unit: params?.unit || '',
  ...
  // On-chain (crypto) settlement opt-in. Off-chain (free) is the default and
  // is the only path unless the deployment is configured AND the user chooses
  // crypto for a BINARY market.
  const onchainAvailable = isOnchainEnabled()
  const [useOnchain, setUseOnchain] = useState(false)
  const [walletOpen, setWalletOpen] = useState(false)
  const onchainWallet = useOnchainWallet()
  ...
  // Step 1: user clicks "Create" — validate, then open the review-before-submit
  // modal. The actual API call happens only after the user confirms in the modal.
  const handleReviewClick = () => {
    // Prevent opening while a submit is in-flight
    ...
    // Clear any previous field errors and open the review step
    setFieldErrors({})
    setIsReviewModalOpen(true)
  }

  // Fold the plain-text resolution criteria into the rich-text description as a
  // clearly-labeled trailing section. The backend has no dedicated field, so
  // this keeps the criteria attached to the market and visible to traders.
  const buildDescriptionPayload = (): JSONContent | string => {
    const base: JSONContent = formState.description ?? {
      type: 'doc',
      content: [],
    }
    const criteria = formState.resolutionCriteria?.trim()
    if (!criteria) return formState.description ?? ''

    const criteriaNodes: JSONContent[] = [
      {
        type: 'paragraph',
        content: [
          { type: 'text', marks: [{ type: 'bold' }], text: 'Resolution criteria' },
        ],
      },
      ...criteria.split('\n').map((line) => ({
        type: 'paragraph',
    ...
    return {
      type: 'doc',
      content: [...(base.content ?? []), ...criteriaNodes],
    }
  }

  // Step 2: user confirmed in the review modal — perform the real create call.
  const handleSubmit = async () => {
    ...
    try {
      ...
      // On-chain (crypto) path: stand up the trustless USDC market by calling
      // the real UmaCtfAdapter.initialize (prepares the CTF condition + requests
      // a UMA price), then tag this contract locally so settlementOf() ===
      // 'onchain'. Failure here shouldn't discard the created off-chain market.
      if (
        useOnchain &&
        onchainAvailable &&
        formState.outcomeType === 'BINARY' &&
        result?.id
      ) {
        try {
          const deployment = await createOnchainMarket({
            question: formState.question.trim(),
          })
          const { registerOnchainMarket } = await import(
            'web/lib/onchain/registry'
          )
          registerOnchainMarket(result.id, {
            conditionId: deployment.conditionId,
            questionId: deployment.questionId,
            positionIds: [
              deployment.positionIds[0].toString(),
              deployment.positionIds[1].toString(),
            ],
          })
          toast.success('Crypto market created on-chain')
        } catch (onchainError: any) {
          console.error('Error creating on-chain market:', onchainError?.message)
          toast.error(
            onchainError?.message ||
              'Market created, but the crypto market could not be set up.'
          )
        }
      }
      ...
      // Reward the creator with a plain, TRUE note: on cpmm markets they earn a
      // real trader bonus (bank -> creator) for every new trader. This is the
      // genuine creator incentive on this platform — there is no percentage cut
      // of trade fees, so we deliberately do not claim one. Skipped for market
      // types that don't pay the bonus (e.g. bounties, polls).
      if (
        result?.id &&
        (result.mechanism === 'cpmm-1' || result.mechanism === 'cpmm-multi-1')
      ) {
        const perTraderBonus = getUniqueBettorBonusAmount(
          result.totalLiquidity ?? 0,
          'answers' in result ? result.answers?.length ?? 0 : 0
        )
        if (perTraderBonus > 0) {
          toast.success(
            `Market created! You earn ${formatMoney(
              perTraderBonus
            )} each time a new trader bets on it — share it to get trading.`,
            { duration: 6000 }
          )
        }
      }
  ...
  return (
    <Col className="min-h-screen">
      ...
      <Col className="mx-auto w-full max-w-3xl gap-6 p-2 pb-6 lg:pb-2">
        {/* Create with AI — drafts markets for the human to review + confirm */}
        <ExpandSection
          title={
            <Row className="items-center gap-2">
              <CubeIcon className="text-primary-600 h-5 w-5" aria-hidden />
              Create with AI
            </Row>
          }
        >
        ...
        {/* Combine existing markets into a shareable parlay (client-side view) */}
        <Link
          href="/parlay"
          className="text-ink-600 hover:text-primary-600 hover:border-primary-300 border-ink-200 bg-canvas-0 flex items-center gap-2 self-start rounded-lg border px-3 py-2 text-sm transition-colors"
        >
          <CollectionIcon className="text-primary-600 h-4 w-4" aria-hidden />
          Or combine markets into a parlay
        </Link>
        ...
        {/* Resolution criteria — surfaced prominently right under the preview,
            because clear criteria are the biggest driver of a healthy market. */}
        {formState.outcomeType && (
          <ResolutionCriteriaSection
            value={formState.resolutionCriteria ?? ''}
            onChange={(value) => updateField('resolutionCriteria', value)}
            outcomeType={formState.outcomeType}
          />
        )}
        ...
        {/* Settlement toggle: Off-chain (free) vs On-chain (crypto). Only for
            BINARY markets, and only when the on-chain deployment is configured.
            Off-chain (free) is always the default. */}
        {onchainAvailable && formState.outcomeType === 'BINARY' && (
          <Col className="gap-2 px-4">
            <Row className="items-center gap-2">
              <CubeIcon className="text-primary-600 h-4 w-4" />
              <span className="text-ink-700 text-sm font-semibold">
                Settlement
              </span>
              <ChoicesToggleGroup
                currentChoice={useOnchain ? 'onchain' : 'offchain'}
                choicesMap={{
                  'Off-chain (free)': 'offchain',
                  'On-chain (crypto)': 'onchain',
                }}
                setChoice={(choice) => setUseOnchain(choice === 'onchain')}
              />
              <InfoTooltip text="Off-chain markets use play money. On-chain markets settle in USDC on Polygon via a self-custodial wallet." />
            </Row>
            {useOnchain && (
              <Row className="items-center gap-2">
                <span className="text-ink-600 text-sm">
                  {onchainWallet.address
                    ? `Wallet: ${onchainWallet.usdcFormatted ?? '—'} USDC`
                    : 'A crypto wallet is required to create this market.'}
                </span>
                <Button
                  color="blue"
                  size="2xs"
                ...
                >
                  {onchainWallet.address ? 'Wallet' : 'Connect wallet'}
                </Button>
              </Row>
            )}
          </Col>
        ...
        <div className="text-ink-500 mt-6 flex items-center justify-center gap-3 pb-0 text-sm">
          <span>© Predikt, Inc.</span>
          <span>•</span>
          <a
            href="https://oracle.markets/terms"
            target="_blank"
          ...
          <span>•</span>
          <a
            href="https://oracle.markets/privacy"
            target="_blank"
            rel="noopener noreferrer"
            className="hover:text-ink-700 underline"
          >
      ...
      <ReviewSubmitModal
        ...
        willBoost={willBoost}
        boostCost={boostCost}
        useOnchain={useOnchain && onchainAvailable}
