...
import { useContractCreatorEarnings } from 'web/hooks/use-creator-earnings'

export function CreatorSharePanel(props: { contract: Contract }) {
  const { contract } = props
  const user = useUser()
  // Real earnings this creator has made on this specific market (trader
  // bonuses). Only queried for the creator viewing their own market.
  const earnedOnThisMarket = useContractCreatorEarnings(
    user?.id === contract.creatorId ? user?.id : undefined,
    contract.id
  )
  return (
      ...
      <Col className="w-full gap-4">
        {user?.id === contract.creatorId && (
          <Row className="text-ink-600 items-center gap-1.5 text-sm">
            ...
            {earnedOnThisMarket !== undefined && earnedOnThisMarket > 0 && (
              <>
                <span aria-hidden>·</span>
                <span className="text-teal-600">
                  earned {formatMoney(earnedOnThisMarket)}
                </span>
              </>
            )}
          </Row>
