...
import {
  isBinaryMulti,
  tradingAllowed,
  type BinaryOrPseudoNumericContract,
  type Contract,
  type ContractParams,
...
import { DepthPanel } from 'web/components/bet/depth-panel'
...
export function ContractPageContent(props: ContractParams) {
  ...
  // Markets with a standalone YES/NO trade box (binary-style). For these we
  // hoist the buy panel into a sticky right-hand column on wide screens
  // (Polymarket-style). Other types keep betting inline with their answers.
  const usesBinaryTradePanel =
    (outcomeType === 'BINARY' ||
      outcomeType === 'PSEUDO_NUMERIC' ||
      outcomeType === 'STONK') &&
    tradingAllowed(liveContract)

  // Reusable trade box — same existing BinaryBetPanel, framed as a card.
  const tradePanel = usesBinaryTradePanel ? (
    <div className="bg-canvas-0 border-ink-200 dark:border-canvas-100 rounded-xl border p-4 shadow-sm">
      <BinaryBetPanel
        contract={liveContract as BinaryOrPseudoNumericContract}
      />
    </div>
  ) : null

  // Real market-depth: CPMM price bar + open-order ladder + recent fills.
  // Shown for binary-style markets even when closed (historical depth).
  const isBinaryStyle =
    outcomeType === 'BINARY' ||
    outcomeType === 'PSEUDO_NUMERIC' ||
    outcomeType === 'STONK'
  const depthPanel = isBinaryStyle ? (
    <DepthPanel
      contract={liveContract as BinaryOrPseudoNumericContract}
      bets={bets}
    />
    ...
    <>
      ...
      {!user && (
        <Row className="bg-canvas-0 sticky top-0 z-50 flex items-center justify-between px-4 py-2 md:hidden">
          <Link
            href="/"
            className="flex items-center gap-1 text-primary-700 dark:text-white"
          >
            <LogoIcon className="h-8 w-8 stroke-primary-700 dark:stroke-white" />
            <span className="text-lg font-thin">Predikt</span>
          </Link>
          <SignUpButton />
        </Row>
      )}
      <Row className="w-full items-start justify-center gap-8">
        ...
        >
          ...
          <Col className={clsx('mb-4 p-4 pt-0 md:pb-8 lg:px-8')}>
            {/* Polymarket-style split: chart + title on the left, sticky
                YES/NO trade box on the right (stacks on mobile). */}
            <Row className="w-full items-start gap-6">
              <Col className="w-full min-w-0 gap-3 lg:gap-4">
                <Col>
                  <div ref={titleRef}>
                    <VisibilityIcon
                ...
                <Col className="gap-2">
                  {/* Chart framed in a clean card container. */}
                  <div className="bg-canvas-0 border-ink-200 dark:border-canvas-100 rounded-xl border p-3 sm:p-4">
                    <ContractOverview
                      contract={liveContract}
                      ...
                      hideBetPanel={usesBinaryTradePanel}
                    />
                  </div>

                  {/* On mobile / narrow screens the trade box sits under the
                      chart; on wide screens it moves to the sticky column. */}
                  {tradePanel && (
                    <div className="lg:hidden">{tradePanel}</div>
                  )}
                  {depthPanel && (
                    <div className="lg:hidden">{depthPanel}</div>
                  )}
              ...
              {/* Sticky right-hand trade column (wide screens only). */}
              {(tradePanel || depthPanel) && (
                <Col className="hidden w-[340px] shrink-0 gap-3 lg:flex">
