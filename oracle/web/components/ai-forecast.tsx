  ...
  return (
    <div className="relative mb-1 w-full">
      <div className="flex items-start">
        ...
        <h3 className="text-med ml-1 pr-2 font-semibold leading-tight text-ink-900 sm:text-lg">
          {title}
        </h3>
      </div>
...
function getCardBgColor(className: string) {
  ...
  switch (cardType) {
    ...
    default:
      ...
      // Default background
      return 'bg-canvas-50 dark:bg-ink-700/20'
  }
}
...
function TitleCard({
  ...
  return (
    <div
      className={`mb-2 w-full rounded-lg bg-canvas-100 p-2 shadow dark:bg-ink-800 sm:p-4 ${className}`}
    >
      <div className="text-center text-lg font-semibold text-ink-800 sm:text-2xl">
        {title}
      </div>
    </div>
  )
      ...
      {orderedSections.map((type, index) => (
        <Col key={type} id={type}>
          ...
          {orderedSections[index - 1] === 'monthly' && type === 'benchmark' && (
            <>
              <div className="mx-auto my-8 h-px w-3/4 bg-ink-200"></div>
          ...
          {orderedSections[index - 1] === 'misuse' && type === 'prize' && (
            <>
              <div className="mx-auto my-8 h-px w-full max-w-4xl bg-ink-200"></div>
              <h2 className="text-primary-600 mb-2 text-xl font-semibold sm:text-2xl">
