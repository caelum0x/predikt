...
export const BuyPanelBody = (
  ...
  const defaultBetAmount = isCashContract
    ? 1
    : quickAddButtonSize === 'small'
    ? 10
    : 50
  // A copy-trade passes the mirrored trade's amount so the panel opens
  // pre-filled with the same stake the followed trader used.
      ...
      <Col className={clsx(className, 'relative rounded-xl px-4 py-2')}>
        ...
        <Row className={'mb-2 mt-2 justify-between'}>
          ...
          <Row className="items-center justify-end gap-2">
            {!isStonk && (
              // Limit orders are an advanced feature — tucked behind a plain
              // text toggle so the default flow stays market-buy simple.
              <button
                type="button"
                onClick={() =>
                  setBetTypeSetting(
                    betTypeSetting === 'Limit' ? 'Market' : 'Limit'
                  )
                }
                className={clsx(
                  'text-sm font-medium transition-colors',
                  betTypeSetting === 'Limit'
                    ? 'text-primary-600 hover:text-primary-700'
                    : 'text-ink-500 hover:text-ink-700'
                )}
              >
                {betTypeSetting === 'Limit' ? 'Hide advanced' : 'Advanced'}
              </button>
            )}
            {onClose && (
              <Button
            ...
            >
              ...
              <Col className="w-full gap-1">
                          ...
                          onClick={() => {
                            toast.success(
                              `Slippage protection on ${
                                isCashContract ? 'cash' : 'play-money'
                              } questions ${
                ...
                {!isStonk &&
                  ...
                  !!betAmount &&
                  currentPayout > 0 && (
                    <Row className="items-baseline justify-between sm:justify-start">
                      <div className="text-ink-600 mr-2 min-w-[120px] whitespace-nowrap">
                        Avg price
                      </div>
                      <span className="text-ink-900 whitespace-nowrap font-medium">
                        {formatPercent(betAmount / currentPayout)}
                      </span>
                    </Row>
                  )}
              </Col>
