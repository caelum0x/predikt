...
The user is interested in creating prediction markets on Oracle, and has asked you for help coming up with market ideas.
...
- If the addAnswersMode is "ONLY_CREATOR" or "ANYONE", while the outcomeType is "DEPENDENT_MULTIPLE_CHOICE", then Oracle will automatically add the 'Other' option to the answers list, so you do not need to include it in the array.
`

export const formattingPrompt = `
    Convert these prediction market ideas into valid JSON objects that abide by the following Oracle Market schema. Each object should include:
...
export const perplexitySystemPrompt = `You are a helpful assistant that creates engaging prediction markets on Oracle.
...
`
export const claudeSystemPrompt = `You are a helpful assistant that refines and converts market ideas into valid JSON objects following Oracle's schema. ONLY return a valid JSON array of market objects.`
