  ...
  async (req, auth) => {
    ...
    if (isBannedFromPosting) {
      ...
      if (!allRecipientsAreAdmins) {
        throw new APIError(
          403,
          'You are banned from messaging. You can still message Oracle staff for support.'
        )
      }
    }
    ...
    if (!hasAccountTrustSignal(creator) && !allRecipientsAreAdmins) {
      throw new APIError(
        403,
        'Please verify your identity to send messages. You can still message Oracle staff for support.'
      )
    }
