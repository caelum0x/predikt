...
async function placeManyBets(apiKey: string, count: number) {
  ...
  const apiKey2 = 'ad55b4c6-794a-4eef-94dc-d80a7438319d' // Oracle
  const betData = {
    contractId: 'pdcWgwpzV4RsJjQGVq9v', // https://dev.oracle.markets/IanPhilips/beeeep-bop
    amount: 1,
    outcome: Math.random() > 0.5 ? 'YES' : 'NO',
  }
  const betData2 = {
    contractId: 'Y9C5Hb9yS8D3C3KRwRYC', // https://dev.oracle.markets/DavidChee/will-sirsalty-hit-infinity-by-this
    amount: 1,
    outcome: Math.random() > 0.5 ? 'YES' : 'NO',
  }
  let success = 0
