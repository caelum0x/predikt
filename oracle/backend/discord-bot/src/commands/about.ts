...
const data = new SlashCommandBuilder()
  .setName('about')
  .setDescription('Learn more about the Oracle bot and how to use it.')

async function execute(interaction: ChatInputCommandInteraction) {
  if (shouldIgnoreMessageFromGuild(interaction.guildId)) return
  await interaction.reply({
    content: `${interaction.client.user.toString()} is the official Discord bot of [Oracle](<${
      config.domain
    }>).
 ...
 **/about** - Learn more about the Oracle bot and how to use it
`,
  })
}
