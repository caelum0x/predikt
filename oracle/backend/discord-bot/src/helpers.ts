...
export const handleButtonPress = async (interaction: ButtonInteraction) => {
  ...
  if (customId === 'question') {
    ...
    const content = `This is a prediction market from [Oracle](<${config.domain}>). You can bet that the event will happen (YES) by reacting with these emojis: ${yesBetsEmojis} and that it won't (NO) with these: ${noBetsEmojis}. The emoji numbers correspond to the amount of mana used, (i.e. your conviction) per bet.`
