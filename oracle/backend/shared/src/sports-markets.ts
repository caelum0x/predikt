...
export async function ensureOfficialGroup(
  ...
  const group = await insert(pg, 'groups', {
    ...
    about: anythingToRichText({ raw: `Official Oracle Sports markets for ${config.name}. Managed by @OracleSports.` }),
    total_members: 1,
...
export async function ensureCommunityAssets(
  ...
  if (existingGroup) {
    ...
    const group = await insert(pg, 'groups', {
      ...
      about: anythingToRichText({ raw: `Community markets for ${config.name}. Curated by Oracle admins.` }),
      total_members: 1,
      privacy_status: 'curated',
    })
    await bulkInsert(pg, 'group_members', [
...
// Post the final score on the market as @OracleSports, mirroring the
...
export async function resolveTournamentMarketsForMatches(
  ...
  if (!creatorUser) throw new Error(`OracleSports user ${creatorId} not found`)
 ...
 *   follower of @OracleSports on every market — but creation always happens in
