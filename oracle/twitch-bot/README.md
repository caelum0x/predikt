...
This sub-repo has everything required to host the Oracle Twitch Bot and associated overlay and dock browser sources for OBS.
...
- **PUBLIC_FACING_URL**: The public URL of the host server, without trailing slash, e.g. https://oracle.markets
...
- **TARGET**: Represents which Oracle site to target and takes one of the following values (default: LOCAL):
  - PROD (https://oracle.markets/)
  - DEV (https://dev.oracle.markets/)
...
- Run the following commands from the root of the Oracle repository:
  - `$ yarn`
  - `$ cd twitch-bot`
  - `$ yarn`
  - `$ yarn dev:fullstack`
...
In order to deploy to the Oracle Twitch servers, you will need to have the appropriate permissions on your Google account, and the following software installed:
...
1.  Go to https://console.cloud.google.com/logs/query?project=oracle and select either `bot-DEV` or `bot-PROD` under `Log name`.
...
- Development: https://dev-twitch-bot.oracle.markets/metrics
- Production: https://twitch-bot.oracle.markets/metrics
