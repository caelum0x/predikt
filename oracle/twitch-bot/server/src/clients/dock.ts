...
import * as OracleAPI from '../manifold-api';
...
export default class DockClient {
  ...
  private async init() {
    ...
    const handshakePacket: PacketHandshakeComplete = {
      actingOracleUserID: this.connectedUser.data.manifoldID,
  ...
  private registerPacketHandlers() {
    ...
    sw.on(PacketRequestResolve, async (p) => {
      ...
      try {
        await OracleAPI.resolveBinaryMarket(currentMarket.data.id, this.connectedUser.data.APIKey, outcome);
      } catch (e) {
        log.trace(e);
      }
    });
    ...
    sw.on(PacketCreateMarket, async (packet: PacketCreateMarket) => {
      try {
        const newMarket = await OracleAPI.createBinaryMarket(this.connectedUser.data.APIKey, packet.question, undefined, 50, { groupID: packet.groupId, visibility: 'unlisted' });
