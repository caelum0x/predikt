...
import { AbstractMarket, NamedBet } from '@common/types/oracle-abstract-types';
import { LiteUser } from '@common/types/oracle-api-types';
import { Bet, CPMMBinaryContract } from '@common/types/oracle-internal-types';
...
export class Market {
  ...
  static async loadFromOracleID(app: App, manifoldID: string, stream: TwitchStream) {
  ...
  async calculateWinners(): Promise<{ user: LiteUser; profit: number }[]> {
    ...
    for (const userID of Object.keys(userProfits)) {
      const user = await Oracle.getUserByID(userID);
