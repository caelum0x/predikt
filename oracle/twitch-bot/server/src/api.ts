...
export default function registerAPIEndpoints(app: App, express: Express) {
  express.post('/unregisterchanneltwitch', async (request, response) => {
    ...
    try {
      const user = app.firestore.getUserForOracleAPIKey(apiKey);
  ...
  express.post('/api/linkInit', async (request, response) => {
    try {
      ...
      if (!(await Oracle.verifyAPIKey(apiKey))) throw new Error('API key invalid.');
  ...
  express.get('/linkAccount', async (request, response) => {
    ...
    try {
      ...
      let user: User;
      try {
        user = app.firestore.getUserForOracleID(sessionData.manifoldID);
      ...
      try {
        await Oracle.saveTwitchDetails(sessionData.apiKey, twitchUser.display_name, user.data.controlToken);
      } catch (e) {
        log.trace(e);
        throw new Error('Failed to save Twitch details to Oracle');
      }
