...
import * as OracleAPITypes from '@common/types/oracle-api-types';
import * as OracleInternalTypes from '@common/types/oracle-internal-types';
...
export async function getUserByID(userID: string): Promise<OracleAPITypes.LiteUser> {
  return <Promise<OracleAPITypes.LiteUser>>(await get(`${MANIFOLD_API_BASE_URL}user/by-id/${userID}`)).json();
}

/**
 * @deprecated Username is volatile. Aim to use user ID instead.
 */
export async function getUserByOracleUsername(manifoldUsername: string): Promise<OracleAPITypes.LiteUser> {
  return <Promise<OracleAPITypes.LiteUser>>(await get(`${MANIFOLD_API_BASE_URL}user/${manifoldUsername}`)).json();
}
...
export async function getUsersStakeInMarket_shares(marketSlug: string, manifoldUsername: string): Promise<{ shares: number; outcome: 'YES' | 'NO' }> {
  return fetch(`${MANIFOLD_API_BASE_URL}bets?market=${marketSlug}&username=${manifoldUsername}`)
    .then((r) => <Promise<OracleInternalTypes.Bet[]>>r.json())
    .then((bets) => {
      let total = 0;
...
export async function createBinaryMarket(
...
): Promise<OracleAPITypes.LiteMarket> {
  ...
  return <Promise<OracleAPITypes.LiteMarket>>(await post(`${MANIFOLD_API_BASE_URL}market`, APIKey, requestData)).json();
}
...
export async function getLatestMarketBets(marketSlug: string, numBetsToLoad?: number): Promise<OracleInternalTypes.Bet[]> {
  return <Promise<OracleInternalTypes.Bet[]>>(await get(`${MANIFOLD_API_BASE_URL}bets?market=${marketSlug}${numBetsToLoad ? `&limit=${numBetsToLoad}` : ''}`)).json();
}

export async function getMarketBySlug(marketSlug: string): Promise<OracleAPITypes.LiteMarket> {
  return <Promise<OracleAPITypes.LiteMarket>>(await get(`${MANIFOLD_API_BASE_URL}slug/${marketSlug}`)).json();
}

export async function getFullMarketByID(marketID: string): Promise<OracleAPITypes.FullMarket> {
  const fullMarket = <OracleAPITypes.FullMarket>await (await get(`${MANIFOLD_API_BASE_URL}market/${marketID}`)).json();
  fullMarket.bets.sort((a, b) => a.createdTime - b.createdTime); // Ensure that bets are oldest-first. The Oracle API doesn't consistently order them.
  return fullMarket;
}

export async function getLiteMarketByID(marketID: string): Promise<OracleAPITypes.LiteMarket> {
  return <Promise<OracleAPITypes.LiteMarket>>(await get(`${MANIFOLD_API_BASE_URL}market/${marketID}/lite`)).json();
}

export async function getGroupBySlug(groupSlug: string): Promise<OracleInternalTypes.Group> {
  return <Promise<OracleInternalTypes.Group>>(await get(`${MANIFOLD_API_BASE_URL}group/${groupSlug}`)).json();
}

export async function saveTwitchDetails(APIKey: string, twitchName: string, controlToken: string): Promise<void> {
  const requestData = {
    twitchInfo: {
      // These names match the variables in Oracle User.ts. DO NOT CHANGE
      twitchName,
      controlToken,
    },
  };
