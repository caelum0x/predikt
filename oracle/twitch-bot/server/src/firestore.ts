...
export default class AppFirestore {
  ...
  getUserForOracleID(manifoldID: string): User {
    const user = this.localUserCache[manifoldID];
    if (!user) throw new UserNotRegisteredException(`No user record for Oracle ID ${manifoldID}`);
    return user;
  }

  getUserForOracleAPIKey(apiKey: string): User {
    for (const manifoldID in this.localUserCache) {
      const user = this.localUserCache[manifoldID];
