import { AnyContractType, Bet, Contract, Group, User } from '@common/types/oracle-internal-types';
...
export default class OracleFirestore {
  ...
  private async validateConnection() {
    const message = 'Accessing Oracle Firestore...';
    try {
      await getDoc(doc(this.contracts, 'dummy'));
    } catch (e) {
      log.error(message + ' failed.');
      throw new Error('Failed to contact Oracle Firestore: ' + e.message);
    }
  ...
  async getOracleUserByOracleID(manifoldID: string, forceLatest = true) {
