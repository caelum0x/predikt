...
import { LiteMarket } from '@common/types/oracle-api-types';
import { Response } from 'node-fetch';
import * as Oracle from './manifold-api';
...
export default class User {
  ...
  public async getBalance(): Promise<number> {
    return (await Oracle.getUserByID(this.data.manifoldID)).balance;
  }
  ...
  async sellAllShares(marketID: string): Promise<Response> {
    return Oracle.sellShares(marketID, this.data.APIKey);
  }
  ...
  public async createBinaryMarket(question: string, description: string, initialProb_percent: number, options?: { visibility?: 'public' | 'unlisted'; groupID?: string }): Promise<LiteMarket> {
    return Oracle.createBinaryMarket(this.data.APIKey, question, description, initialProb_percent, options);
  }

  public async resolveBinaryMarket(marketID: string, outcome: ResolutionOutcome) {
    return Oracle.resolveBinaryMarket(marketID, this.data.APIKey, outcome);
  }

  public async placeBet(marketID: string, amount: number, yes: boolean): Promise<Response> {
    return Oracle.placeBet(marketID, this.data.APIKey, amount, yes ? 'YES' : 'NO');
  }
}
