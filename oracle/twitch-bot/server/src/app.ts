...
import OracleFirestore from './oracle-firestore';
...
export default class App {
  ...
  readonly manifoldFirestore: OracleFirestore;
  ...
  public constructor() {
    ...
    this.manifoldFirestore = new OracleFirestore();
  ...
  public async getDisplayNameForUserID(userID: string) {
    ...
    try {
      const user = this.firestore.getUserForOracleID(userID);
      displayName = user.data.twitchLogin;
    } catch {
      try {
        const user = await this.manifoldFirestore.getOracleUserByOracleID(userID);
