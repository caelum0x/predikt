// Make the deploy-kit runnable without a separate install.
//
// The kit's FPMM factory step is plain Node + viem (Solidity ^0.5.1 can't share
// the ^0.8 forge-std Script the other two use, exactly as fpmm/deploy-fpmm.mjs).
// Rather than duplicate a full install we reuse the sibling predikt-relay's
// already-installed node_modules by symlinking the exact packages viem needs
// into this folder's node_modules. Idempotent — run it as often as you like.
...
// deploy-kit/ -> predikt-contracts/ -> prediction/ -> predikt-relay
