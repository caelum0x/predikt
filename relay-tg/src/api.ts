// Real fetch-based client for the Predikt/oracle backend.
//
// Ported from herald (the Discord companion bot) so the Telegram bot talks to
// exactly the same backend the same way. The base URL is env-configurable via
// ORACLE_API_URL so this bot can run against prod or dev without a code change.
//   Prod: https://api.oracle.markets/v0   Dev: https://api.dev.oracle.markets/v0
//
// Node 18+ ships a global `fetch`, so no HTTP dependency is required.
 ...
 *
 * Mirrors herald's OracleClient so the Telegram and Discord bots share identical
 * backend behaviour. Reads the host from `ORACLE_API_URL`.
 */
export class OracleClient {
  ...
  // GET /bets?userId=... -> Bet[]  (used by /portfolio)
  getBets({
    userId,
    contractId,
    limit,
...
/**
 * Factory that mirrors herald's. Reads `ORACLE_API_KEY` from the environment
 * when no key is passed (used for unauthenticated market reads).
 */
